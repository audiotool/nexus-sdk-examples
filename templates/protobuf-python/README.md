...
It sets them up for the language of python, but this template can be adjusted to any language
...
To use these services, you have to generate code for your language of choice, that generates bindings based on the `.proto` files which you can then use in your code. 


In this example project, we generate bindings for the language of python.
...
* in [protos](./protos/), we pasted the protobuf files downloaded from the [dev plage](https://developer.audiotool.com/explore-protobufs). In there you also find:
    * `buf.yaml`: general buf.build toolchain config
    * `buf.gen.yaml`: contains the links to the _python code generators_, and the output directory  to which the generated files should be written to
...
* update the generators listed in [buf.gen.yaml](./protos/buf.gen.yaml) to the ones of your language of choice. Currently there's 3 in there, you can remove them all, and add those for you language of choice from the [buf.build plugins list](https://buf.build/plugins/protobuf).
